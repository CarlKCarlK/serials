...
macro_rules! define_led_strips {
        ...
        paste::paste! {
            ...
            $(
                #[doc = concat!(
                    ...
                    "Created with [`", stringify!($group), "::new_shared`]."
                )]
            ...
            // Generate the group marker struct with new_shared() constructor
            pub struct $group;
                    ...
                    Ok((
                        $(
                            [<$label:camel LedStrip>]::new(
                                define_led_strips!(@__select_sm $sm_index, sm0_wrapped, sm1_wrapped, sm2_wrapped, sm3_wrapped),
                                [<$label _dma>],
                                [<$label _pin>],
                                spawner
                            )?,
    ...
    (@__inc_counter pio: $pio:ident, group: $group:ident, sm: 0, strips_out: [$($out:tt)*], strips_in: [$($in:tt)*]) => {
        define_led_strips! { @__with_defaults pio: $pio, group: $group, sm_counter: 1, strips_out: [$($out)*], strips_in: [$($in)*] }
    };
    (@__inc_counter pio: $pio:ident, group: $group:ident, sm: 1, strips_out: [$($out:tt)*], strips_in: [$($in:tt)*]) => {
        define_led_strips! { @__with_defaults pio: $pio, group: $group, sm_counter: 2, strips_out: [$($out)*], strips_in: [$($in)*] }
    };
    (@__inc_counter pio: $pio:ident, group: $group:ident, sm: 2, strips_out: [$($out:tt)*], strips_in: [$($in:tt)*]) => {
        define_led_strips! { @__with_defaults pio: $pio, group: $group, sm_counter: 3, strips_out: [$($out)*], strips_in: [$($in)*] }
    };
    (@__inc_counter pio: $pio:ident, group: $group:ident, sm: 3, strips_out: [$($out:tt)*], strips_in: [$($in:tt)*]) => {
        define_led_strips! { @__with_defaults pio: $pio, group: $group, sm_counter: 4, strips_out: [$($out)*], strips_in: [$($in)*] }
    };
}
