...
fn check_all() -> ExitCode {
    ...
    if !run_command(
        Command::new("cargo")
            ...
            .args(["build", "--lib", "--features", "pico2,arm", "--no-default-features"]),
    ) {
        return ExitCode::FAILURE;
    }

    println!("\n{}", "==> Building examples (pico2, arm, no wifi)...".cyan());
    let examples_no_wifi = ["blinky", "ir", "led_strip", "led_strip_snake", "led24x4_clock"];
...
fn build_lib(board: Board, arch: Arch, wifi: bool) -> ExitCode {
    ...
    if run_command(
        Command::new("cargo")
            .current_dir(&workspace_root)
            .args(["build", "--lib", "--features", &features, "--no-default-features"]),
    ) {
...
fn build_uf2(name: &str, board: Board, arch: Arch, wifi: bool) -> ExitCode {
    ...
    if run_command(
        ...
        println!(
            "{}",
            format!("UF2 created: {uf2_path} 🚀").green().bold()
        );
        ...
        println!(
            "{}",
            "Note: Install elf2uf2-rs with: cargo install elf2uf2-rs"
                .yellow()
        );
        ExitCode::FAILURE
    }
