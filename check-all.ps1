...
cargo build --lib --features pico2
...
foreach ($example in $examples_no_wifi) {
    ...
    cargo build --example $example --features pico2
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
...
foreach ($example in $examples_wifi) {
    Write-Host "  - $example" -ForegroundColor DarkGray
    cargo build --example $example --features pico2,wifi
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
...
# Note: Using pico2,wifi for docs instead of pico1,wifi because embassy-rp from git main
...
cargo doc --no-deps --features pico2,wifi
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
